...
const STATE_COPY: Record<ProvState, { title: string; tone: string; ring: string; sub: string }> = {
  verified: {
...
export function ProvenanceCard({ provenance, tierLabel }: ProvenanceCardProps) {
  ...
  return (
    <Panel title="Provenance" aside={<Tag tone="accent">{tierLabel}</Tag>}>
      <div className={cn("rounded-lg border px-4 py-3", copy.ring)}>
        ...
        {provenance.detail && (
          <p className="mt-2 rounded bg-black/20 px-2 py-1 font-mono text-xs text-slate-300">
            {provenance.detail}
          </p>
        )}
      </div>

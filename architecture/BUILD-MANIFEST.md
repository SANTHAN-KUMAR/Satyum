...
- **Cop-out guard (must-fail CI):** Risk: degrading to 'a signature exists' (signature.intact only) so an attacker's self-signed cert is treated as source-verified; ignoring /ByteRange so appended-content (incremental-update) tamper passes; hardcoding 'valid' when OCSP is offline. Guard: two adversarial fixtures - (a) PDF signed with attacker's own cert -> chain-to-anchor fails, so it MUST NOT source-verify (it is NOT_EVALUATED as 'issuer unconfirmed' and routed to review; deliberately NOT labelled 'tampered', since the bytes are intact and a genuine unpinned-issuer document is cryptographically identical - see ADR-006); (b) validly-signed PDF with bytes appended after ByteRange -> digest mismatch -> MUST fail as tampered. Fail closed on unreachable revocation. The invariant: neither is ever treated as source-verified or approved.
...
- **UX-hardening update:** the challenge TTL (`backend/app/routes/verify.py::_CHALLENGE_TTL_MS`) is now server-enforced as a time backstop alongside the frame-count trigger — previously it only drove the on-screen countdown while a fixed frame count silently decided scoring, so a cooperating user could be scored before the honest deadline passed. A bounded in-session retry (`_MAX_CHALLENGE_RETRIES = 3`) lets a genuine document-holder recover from a bad take (bad lighting, wrong angle) without a full socket/camera teardown; this is a UX/session-cost bound, not a security control, since a photo-of-screen or pre-recorded clip fails single-homography-consistency on every independent attempt regardless of retry count. A multi-step chained challenge (§ note above the axes constants in `challenge.py`) remains a tracked follow-up for deeper anti-replay entropy.
